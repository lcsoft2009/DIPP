...
//Ensure this node is unique, otherwise start over			
	...
	//		If the node was unique, save it in a list of nodes and repeat until the list contains n nodes
		...
		if(hosts.size()>1)
			{
			System.out.println("First n-1("+(hosts.size()-1)+") nodes each responsible for " + span + " images");
			System.out.println("Last node responsible for " + last + " images");
			}

		else
			System.out.println("The only one node responsible for " + span + " images");
