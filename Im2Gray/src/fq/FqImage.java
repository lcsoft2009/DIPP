...
//			System.out.println("sumh"+sumh+","+"sums"+sums+","+"sumv"+sumv);
	...
	public FqImage( FloatImage imh ){
		this.imh	=	imh;
		this.height	=	imh.getHeight();
		this.width	=	imh.getWidth();
...
raster = colorModel.createCompatibleWritableRaster(width,
		...
		for( i = 0 ; i < this.width ; i++ ){
			for( j = 0 ; j < this.height ; j++ ){
				...
				;
				this.points[i][j].setRGB( colorModel.getRGB(raster.getDataElements(i, j, null)) );
