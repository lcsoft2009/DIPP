...
public class JpegFromHibInputFormat extends
		FileInputFormat<NullWritable, BytesWritable> {

	/**
	 * Returns an object that can be used to read records of type
	 * ImageInputFormat
	 */
	@Override
	...
	public List<InputSplit> getSplits(JobContext job) throws IOException {
		...
		for (FileStatus file : listStatus(job)) {
			...
			BlockLocation[] blkLocations = fs.getFileBlockLocations(hib
					.getDataFile(), 0, offsets.get(offsets.size() - 1));
			System.out.println("blkLocations.length="+blkLocations.length);
			if (numMapTasks == 0) 
			{
					int i = 0, b = 0;
			...
			else 
			{
				...
				System.out.println("imageRemaining: " + imageRemaining
						+ "\ttaskRemaining: " + taskRemaining
						+ "\tlastOffset: " + lastOffset + "\ti: " + i);
				while (imageRemaining > 0) 
				{
					...
					int endIndex = getBlockIndex(blkLocations,
							currentOffset - 1);
					...
					splits.add(new FileSplit(hib.getDataFile().getPath(),
							lastOffset, currentOffset - lastOffset, hosts
